...
ARBITER_RATIO = 1.0015
UPDATE_SLEEP_INTERVAL = 1
...
class Executer:
    ...
    def _calculate_ratios(instrumentos: List[Dict]) -> None:
        ...
        for item in instrumentos:
            item["USD_a_pesos"] = (
                float(min(item["prCompraPesos"], item["cierreAnteriorPesos"]) / item["prVentaDolar"])
                if item.get("prCompraPesos") and item.get("prVentaDolar") and item.get("cierreAnteriorPesos")
                else None
            )
            item["pesos_a_USD"] = (
                float(item["prVentaPesos"] / min(item["prCompraDolar"], item["cierreAnteriorDolar"]))
                if item.get("prVentaPesos") and item.get("prCompraDolar") and item.get("cierreAnteriorDolar")
                else None
            )
    ...
    def execute(
        ...
        if not al30:
            ...
            return
        pr_compra_pesos_AL30 = al30.get("prCompraPesos")
        pr_venta_pesos_AL30 = al30.get("prVentaPesos")
        pr_compra_dolar_AL30 = al30.get("prCompraDolar")
        pr_venta_dolar_AL30 = al30.get("prVentaDolar")

        if not pr_compra_pesos_AL30 or not pr_venta_pesos_AL30 or not pr_compra_dolar_AL30 or not pr_venta_dolar_AL30:
            logger.warning("No se pudo calcular ratios para AL30. Verifica datos.")
            return
        else:
            USD_a_pesos_AL30 = pr_compra_pesos_AL30 / pr_venta_dolar_AL30
            pesos_a_USD_AL30 = pr_venta_pesos_AL30 / pr_compra_dolar_AL30
        ...
        for item in instrumentos:
            if item["pesos_a_USD"] is None or item["pesos_a_USD"] <= 1:
                ...
                if item["pesos_a_USD"] * ratio < USD_a_pesos_AL30:
                    ...
                    logger.info(
                        f"Ejecutando estrategia para {item['ticker']} → quant={quant}"
                    )
                    ...
                    self.comprar_en_pesos(item, quant)
            ...
            if item["USD_a_pesos"] is None or item["USD_a_pesos"] <= 1:
                ...
                if pesos_a_USD_AL30 * ratio < item["USD_a_pesos"]:
                    ...
                    self.comprar_en_dolares(item, quant)


    def comprar_en_pesos(self, dolarizador: Dict, quant: int, order_type: str = "LIMIT"):
    ...
    def comprar_en_dolares(self, pesificador: Dict, quant: int, order_type: str = "LIMIT"):
