...
class DataFrameHandler:
    ...
    def create_df(self):
        return pd.DataFrame(
            ...
            columns=[
                "ticker",
                "tickerD",
                "volCompraPesosprCompraPesos",
                "prVentaPesos",
                "volVentaPesos",
                "volCompraDolarprCompraDolar",
                "prVentaDolar",
                "volVentaDolar",
            ],
        )
    ...
    def update_df(self, data):
        for r in data:
            values = str(r).split("|")
            self.update_row(values)

        if data.count() == 1 and "AL30" not in data[0]:
            # operar
            pass
    ...
    def update_row(self, values):
        ...
        self.df.loc[
            ...
            ["volCompraPesos", "prCompraPesos", "prVentaPesos", "volVentaPesos"],
        ] = np.array([values[2], values[3], values[4], values[5]]).astype(float)
        self.df.loc[
            self.df.tickerD == ticker,
            ["volCompraDolar", "prCompraDolar", "prVentaDolar", "volVentaDolar"],
        ] = np.array([values[2], values[3], values[4], values[5]]).astype(float)
...
class Executer:
    ...
    def execute(self):
        ...
        pesos_a_USD_AL30 = self.df[
            self.df.ticker == "AL30" and self.df.pesos_a_USD > 1
        ].pesos_a_USD
        USD_a_pesos_AL30 = self.df[self.df.ticker == "AL30"].USD_a_pesos

        # Verifico que el maximo entre "USD a pesos" o "USDCI a pesos" sea mayor a "pesos a USD" (multiplicado por el ratio)
        if pesos_a_USD_AL30 * ratio < USD_a_pesos_MAX:
            print("Arbitraje: Comprar ticker en USD y vender ticker en pesos. \n")
            (
                ticker,
                tickerD,
                volCompraPesos,
                ...
                self.df.USD_a_pesos == USD_a_pesos_MAX,
                [
                    "ticker",
                    "tickerD",
                    "volCompraPesosprCompraPesos",
            ...
            if volCompraPesos >= max_vol and volVentaDolar >= max_vol:
                r = requests.post(
                    "https://matriz.cocos.xoms.com.ar/api/v2/accounts/35315/orders/single",
                    headers={"Accept": "application/json", "Cookie": cookie},
                    json={
                        "account": account,
                        "price": prVentaDolar,
                        "quantity": max_vol,
                        "securityId": f"bm_MERV_{tickerD}_24hs",
                        "totalAmount": max_vol * prVentaDolar / 100,
                        "side": "1",
                        "ordType": "2",
                        "execInst": [],
                        "canReplacePreviousOrders": True,
                        "timeInForce": "0",
                        "isQuantity": True,
                        "sent": True,
                        "ignoredAlertPrice": True,
                        "visibleAlertPrice": True,
                        "frontId": "1748005415754",
                    },
                )
            data = simplejson.loads(r.text)
            order_id = data["orderId"]
            order_status = data["ordStatus"]

            print("Arbitraje: Comprar AL30 en pesos y vender AL30 en USD. \n")
            pass
        elif pesos_a_USD_Min * ratio < USD_a_pesos_AL30:
            print("Arbitraje: Comprar AL30 en USD y vender AL30 en pesos. \n")
            print("Arbitraje: Comprar ticker en pesos y vender ticker en USD. \n")
            pass
        else:
